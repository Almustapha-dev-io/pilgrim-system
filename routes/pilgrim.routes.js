...
const { initiator, reviewer, initiator_admin, initiator_reviewer, 
    admin_reviewer, initiator_reviewer_admin } = require('../middleware/role');
...
router.get('/by-code-and-lga/:code/:id', [auth, reviewer, validateObjectId], async (req, res) => {
    try {
...
// get pilgrim by code
router.get('/by-code/:code', [auth, initiator_admin], async (req, res) => {
    try {
...
router.get('/by-lga/:id', [auth, reviewer, validateObjectId], async (req, res) => {
    try {
...
router.get('/by-lga', [auth, initiator_admin], async (req, res) => {
    try {
        const pilgrims = await Pilgrim
            .find({ deleted: false, 'enrollmentDetails.enrollmentZone': req.userLga })
            .populate('enrollmentDetails.enrollmentZone', '_id name code')
            .populate('personalDetails.stateOfOrigin', '_id name')
            .populate('personalDetails.localGovOfOrigin', '_id name')
            .populate('createdBy', '_id name')
...
router.get('/', [auth, reviewer], async (req, res) => {
    try {
        const pilgrims = await Pilgrim
            .find({ deleted: false })
            .populate('enrollmentDetails.enrollmentZone', '_id name code')
...
router.get('/by-year/:year', [auth, reviewer], async (req, res) => {
    try {
        const year = req.params.year;

        if (!mongoose.Types.ObjectId.isValid(year)) return res.status(400).send('Invalid year')
        const pilgrims = await Pilgrim
            .find({ deleted: false, 'enrollmentZone.enrollmentYear': year })
            .populate('enrollmentDetails.enrollmentZone', '_id name code')
            .populate('personalDetails.stateOfOrigin', '_id name')
            .populate('personalDetails.localGovOfOrigin', '_id name')
            .populate('createdBy', '_id name')
...
//  get deleted pilgrims by lga
router.get('/deleted/by-lga/:id', [auth, reviewer, validateObjectId], async (req, res) => {
    try {
        const { id: lgaId } = req.params;

        const pilgrims = await Pilgrim
            .find({ deleted: true, 'enrollmentDetails.enrollmentZone': lgaId })
            .populate('enrollmentDetails.enrollmentZone', '_id name code')
            .populate('personalDetails.stateOfOrigin', '_id name')
            .populate('personalDetails.localGovOfOrigin', '_id name')
            .populate('createdBy', '_id name')
...
//  get deleted pilgrims by lga
router.get('/deleted/by-lga', [auth, admin], async (req, res) => {
    try {
        const pilgrims = await Pilgrim
            .find({ deleted: true, 'enrollmentDetails.enrollmentZone': req.userLga })
            .populate('enrollmentDetails.enrollmentZone', '_id name code')
            .populate('personalDetails.stateOfOrigin', '_id name')
            .populate('personalDetails.localGovOfOrigin', '_id name')
            .populate('createdBy', '_id name')
...
router.get('/deleted', [auth, reviewer], async (req, res) => {
    try {
        const pilgrims = await Pilgrim
            .find({ deleted: false })
            .populate('enrollmentDetails.enrollmentZone', '_id name code')
