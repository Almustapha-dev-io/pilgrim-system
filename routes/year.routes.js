...
router.get('/allocated-seats-for-lga/:id', [auth, validateObjectId], async (req, res) => {
    ...
    const year = await Year
        .findById(req.params.id)
        .populate('seatAllocations.zone', '_id name code');

    if (!year) return res.status(404).send('Year with given ID not found.');
    
    const fetchedZone = year.seatAllocations.find(a => a.zone._id.toString() === req.body.zoneId.toString());
    if (!fetchedZone) return res.status(404).send('Selected zone not in seat allocations list.');

    res.send(fetchedZone);
});
...
router.post('/', [auth, superAdmin], async (req, res) => {
    ...
    if (seatAllocations.length > 0) {
        const validObjectIDs = seatAllocations.every(lg => mongoose.Types.ObjectId.isValid(lg.zone));
        if (!validObjectIDs) return res.status(400).send('Invalid Zone ID');
    }
    
    let fetchedYear = await Year.findOne({ year: req.body.year });
    if (fetchedYear) return res.status(400).send(`${fetchedYear.year} Hajj year already opened.`);

    fetchedYear = new Year({ year, active, dateOpened, lastClosed, seatAllocations });

    fetchedYear = await fetchedYear.save();
    res.send(fetchedYear);
});
...
*/
router.put('/add-allocation/:id', [auth, superAdmin, validateObjectId], async (req, res) => {
...
router.put('/edit-seat-allocation/:id', [auth, validateObjectId], async (req, res) => {
    const { error } = validateForUpdate(req.body);
    if (error) return res.status(400).send(error.details[0].message);

    const { zone, seatsAllocated } = req.body;
    if (!zone || !seatsAllocated) return res.status(400).send('Please provide all required parameters');
    ...
    let zoneIndex = year.seatAllocations.findIndex(z => z.zone.toString() === zone.toString());
    if (zoneIndex === -1) return res.status(400).send('Provided zone has not been allocated any seats.');
 ...
 */
router.put('/reopen-hajj-year/:id', [auth, superAdmin, validateObjectId], async (req, res) => {    
    const year = await Year.findByIdAndUpdate(req.params.id, {
        $set: { active: true }
...
router.put('/close-hajj-year/:id', [auth, /* superAdmin, */ validateObjectId], async (req, res) => {
    const year = await Year.findByIdAndUpdate(req.params.id, {
