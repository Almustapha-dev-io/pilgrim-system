...
const { Allocation, validateForUpdate, validate } = require('../models/allocation.model');
...
router.get('/:id', [auth, initiator_admin, validateObjectId], async (req, res) => {
    ...
    if (!allocation) return res.status(404).send('Allocation with the given number has not been registered in your zone.');

    return res.send(allocation);
});

router.get('/year/:id/:type', [auth, initiator_admin, validateObjectId], async (req, res) => {
    ...
    res.send({ totalDocs, allocations });
});

router.get('/zone/:id/year/:yearId/:type', [auth, reviewer, validateObjectId], async (req, res) => {
    const pageSize = +req.query.pageSize || 5;
    const page = +req.query.page || 1;

    if (!mongoose.Types.ObjectId.isValid(req.params.yearId)) res.status(400).send('Invalid year ID.');
...
router.put('/add-payment/:id', [auth, validateObjectId], async (req, res) => {
    ...
    if (!allocation) return res.status(404).send('Allocation with given ID not found');
    ...
    const duplicateTransactions = allocation.paymentHistory.filter(a => paymentHistory.find(b => {
        return (a.tellerNumber === b.tellerNumber) || (a.receiptNumber === b.receiptNumber)
    }));
    if (!duplicateTransactions.length) return res.status(400).send('Duplicate transactions were spotted.');
...
router.put('/migrate', auth, async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(allocationId)) return res.status(400).send('Invalid allocation id');
    ...
    const previousAllocation = await Allocation.findById(allocationId).populate('pilgrim');
    if (!previousAllocation) return res.status(400).send('Allocation not found');

    if (!previousAllocation.pilgrim) return res.status(404).send('Pilgrim with given ID not found.');
    if (previousAllocation.deleted) return res.status(400).send('Cannot migrate allocation flagged for deletion.');
    ...
    if (pilgrimAllocated) return res.status(400).send('Pilgrim already allocated for the selected hajj year.');

    const seatAllocations = year.seatAllocations;
    if (!seatAllocations) return res.status(400).send('No allocations for this year');

    const zoneAllocations = seatAllocations.find(al => al.zone.toString() === previousAllocation.enrollmentZone.toString());
    if (!zoneAllocations) return res.status(400).send('No seat allocations for this registration zone. Contact admin');

    if (zoneAllocations.seatsAllocated < allocationNumber) {
        return res.status(400).send(`Only ${zoneAllocations.seatsAllocated} allocations are allowed for this registration center.`)
    }

    const zone = await LocalGovernment.findById(previousAllocation.enrollmentZone);
    ...
    if (seat) return res.status(400).send('The selected allocation number is unavailable');

    const passportExpiryDate = new Date(previousAllocation.pilgrim.passportDetails.expiryDate).getTime();
    ...
    res.send({ allocation, previousAllocation });
});
...
router.post('/', [auth, initiator], async (req, res) => {
    ...
    if (existing) return res.status(400).send('Pilgrim already allocated for the year. Please update');

    const year = await Year.findOne({ active: true, _id: enrollmentYear });
    if (!year) {
        return res.status(400).send(`Hajj registration for ${enrollmentYear} not opened yet.`);
    }
    ...
    const yearAllocations = year.seatAllocations.find(lg => lg.zone.toString() === zone._id.toString());
    if (!yearAllocations) return res.status(400).send('Seats not allocated to zone');

    const allocatedSeats = yearAllocations.seatsAllocated;
    if (!allocatedSeats) return res.status(400).send('Seats not allocated to selected Center');
    if (allocatedSeats < enrollmentAllocationNumber) return res.status(400).send('Invalid allocation number.');
...
router.put('/:id/restore', [auth, validateObjectId], async (req, res) => {
    ...
    const allocation = await Allocation.findById(req.params.id).populate('pilgrim');
    if (!allocation) return res.status(400).send('Invalid allocation id.');

    if (allocation.code || allocation.enrollmentAllocationNumber || !allocation.deleted) {
        return res.status(400).send('This allocation has been allocated a seat. To change please unassign seat');
    }
    ...
    const zoneAllocations = year.seatAllocations.find(lg => lg.zone.toString() === zone._id.toString());
    if (!zoneAllocations) return res.status(400).send('Seats not allocated to specified Center.');
    ...
    if (existingAllocation) return res.status(400).send('Allocation number unavailable');
...
router.delete('/:id', [auth, admin_reviewer, validateObjectId], async (req, res) => {
    ...
    allocation = await Allocation.findByIdAndUpdate(allocation._id, { $set: {
        deleted: true,
        deletionReason,
        code: undefined,
